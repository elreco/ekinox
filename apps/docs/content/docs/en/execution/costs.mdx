...
Ekinox automatically calculates costs for all workflow executions, providing transparent pricing based on AI model usage and execution charges. Understanding these costs helps you optimize workflows and manage your budget effectively.
...
<Tabs items={['Hosted Models', 'Bring Your Own API Key']}>
  <Tab>
    **Hosted Models** - Ekinox provides API keys with a 2.5x pricing multiplier:
...
**Example Request:**
```bash
curl -X GET -H "X-API-Key: YOUR_API_KEY" -H "Content-Type: application/json" https://www.ekinox.app/api/users/me/usage-limits
```
...
Ekinox uses a **base subscription + overage** billing model:

### How It Works

**Pro Plan ($30/month):**
- Monthly subscription includes $30 of usage
- Usage under $30 → No additional charges
- Usage over $30 → Pay the overage at month end
- Example: $45 usage = $30 (subscription) + $15 (overage)
...
When unbilled overage reaches $50, Ekinox automatically bills the full unbilled amount.
...
- Review your current usage in [Settings → Subscription](https://www.ekinox.app/settings/subscription)
