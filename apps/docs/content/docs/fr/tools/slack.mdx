...
<iframe
  ...
  title="Intégration de Slack avec Ekinox"
  frameBorder="0"
...
- **Automatiser les notifications des agents** : envoyer des mises à jour en temps réel depuis vos agents Ekinox vers n'importe quel canal Slack
- **Créer des points de terminaison webhook** : configurer des bots Slack comme webhooks pour déclencher des workflows Ekinox à partir d'activités Slack
...
Dans Ekinox, l'intégration Slack permet à vos agents d'interagir de manière programmatique avec Slack de plusieurs façons dans le cadre de leurs flux de travail :
