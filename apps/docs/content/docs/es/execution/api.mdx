...
Ekinox proporciona una API externa completa para consultar registros de ejecución de flujos de trabajo y configurar webhooks para notificaciones en tiempo real cuando los flujos de trabajo se completan.
...
Puedes generar claves de API desde la configuración de usuario en el panel de control de Ekinox.
...
Recibe notificaciones en tiempo real cuando se completan las ejecuciones de flujos de trabajo. Los webhooks se configuran a través de la interfaz de usuario de Ekinox en el editor de flujos de trabajo.
...
Cuando se completa la ejecución de un flujo de trabajo, Ekinox envía una solicitud POST a tu URL de webhook:
...
- `ekinox-event`: Tipo de evento (siempre `workflow.execution.completed`)
- `ekinox-timestamp`: Marca de tiempo Unix en milisegundos
- `ekinox-delivery-id`: ID único de entrega para idempotencia
- `ekinox-signature`: Firma HMAC-SHA256 para verificación (si se configura un secreto)
- `Idempotency-Key`: Igual que el ID de entrega para detección de duplicados

### Verificación de firma

Si configuras un secreto de webhook, verifica la firma para asegurar que el webhook proviene de Ekinox:
...
2. **Seguridad de webhook**: Siempre configura un secreto de webhook y verifica las firmas para asegurar que las solicitudes provienen de Ekinox.
