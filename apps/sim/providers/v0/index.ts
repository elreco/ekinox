...
const logger = createLogger('V0Provider')
...
export const v0Provider: ProviderConfig = {
  id: 'v0',
  name: 'v0',
  description: 'v0 by Vercel - AI models for modern web development',
  version: '1.0.0',
  models: getProviderModels('v0'),
  defaultModel: getProviderDefaultModel('v0'),

  executeRequest: async (
    request: ProviderRequest
  ): Promise<ProviderResponse | StreamingExecution> => {
    logger.info('Preparing v0 request', {
    ...
    if (tools?.length) {
      preparedTools = prepareToolsWithUsageControl(tools, request.tools, logger, 'v0')
      ...
      if (filteredTools?.length && toolChoice) {
        ...
        logger.info('v0 request configuration:', {
    ...
    try {
      if (request.stream && (!tools || tools.length === 0)) {
        logger.info('Using streaming response for v0 request')
      ...
      logger.error('Error in v0 request:', {
        error,
        duration: totalDuration,
      })
